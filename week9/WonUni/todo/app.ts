...
fetch(API_URL)        //API_URL에 get 요청을 보내 To-do 리스트 데이터를 가져옴, promise 객체를 반환. 응답이 도착하면 .then()메소드로 이어짐.
  ...
  .then((data: Todo[]) => renderTodo(data));// JSON형식으로 변환된 배열 데이터를 화면에 표시.
...
const addTodo = ():void => {
  ...
  fetch(API_URL, {
    ...
    body: JSON.stringify({ ...newTodo}),   //JSON.stringify()는 자바스크립트 객체나 배열을 JSON 문자열로 변환하는 함수, ...newTodo는 newTodo 객체의 모든 프로퍼티를 새 객체에 복사하는 역할,새 객체에 completed: false라는 새로운 프로퍼티가 추가
  })
    .then((response) => response.json())
    .then(() => {
